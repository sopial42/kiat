...
- **RLS is enabled AND forced on any table that holds user-scoped data.** Both `ALTER TABLE X ENABLE ROW LEVEL SECURITY` and `ALTER TABLE X FORCE ROW LEVEL SECURITY` must be present. Without `FORCE`, the table owner (typically the migration runner) silently bypasses RLS regardless of policies. Missing either is a blocker.
- **RLS policies reference the project's documented JWT-claim mechanism correctly.** The mechanism is project-specific — vanilla PostgreSQL projects use `current_setting('request.jwt.claim.sub', true)::uuid`, Supabase projects use `auth.uid()`, Hasura projects use `current_user_id()`. The reviewer's job is to check that the policy predicate matches whatever `delivery/specs/database-conventions.md` declares as canonical for this project, AND that the predicate compares to the right column (`user_id` or equivalent), not a hardcoded value or NULL-returning expression.
- **The runtime database role is non-superuser, non-BYPASSRLS.** `ENABLE` + `FORCE` + a correct policy are necessary but not sufficient — if the backend connects at runtime as a superuser or `BYPASSRLS` role, RLS is silently inert. Check that the deployment spec's `DATABASE_URL` (or equivalent) names a `NOSUPERUSER NOBYPASSRLS` role, and that repository code switches to it via `SET LOCAL ROLE` (or equivalent project-canonical pattern) inside every per-request transaction.
