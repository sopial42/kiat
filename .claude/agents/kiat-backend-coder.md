...
- **Q1 — Observable?** Would a user, an operator reading prod logs/metrics, or an automated check (CI, security scanner) see a different state if you didn't address this? "A future maintainer might be confused" does **NOT** count. If no → don't write the deviation. If yes → continue.
- **Q2 — > 10 min?** If the fix is ≤10 min combined coder + reviewer (small test, sub-30-line rename, one-bullet doc), **apply it inline in this PR** and report it as L1 in the deviation block. The producer pays the cost while the file is warm.
...
Only deviations that are observable + >10 min + no piggyback become L2 queue entries. If your `Business Deviations:` block is filling with cosmetic-naming, missing-test, or doc-staleness items classified as L2, the gate is being misapplied — re-run it. The reviewer will flag weakly-justified L2 entries.
