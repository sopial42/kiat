...
- **Skills**: which contextual skills from `available-skills.md` apply? See **Step 4.5** below for the explicit routing rules.
...
### 4.5. Route contextual skills (explicit per-skill rules)

For each skill below, decide ADD or SKIP based on the **trigger** column. Apply each rule independently — a story can pull zero, one, or several. **Default is SKIP** when the match is ambiguous: the coder can escalate later if the skill turns out to be needed, but a skill listed for nothing inflates the coder's context budget for nothing.

| Skill | ADD when… | SKIP when… |
|---|---|---|
| `kiat-ui-ux-search` | New visual component designed from scratch / palette-typography-spacing decision beyond `design-system.md` / accessibility-critical interface (forms, navigation, data tables) / chart or data viz / animation pattern | Pure backend / uses an existing pattern already in `project-memory.md` / designer provided explicit Figma reference / trivial UI tweak (label/color/text) |
| `frontend-design` | Significant visual surface (landing page, marketing component, dashboard layout, hero section) AND no live Figma/screenshot exists in `### Mockups` to drive aesthetic decisions | Story has a binding visual reference (Figma URL or static screenshot) — that's the source of truth, not a generic design skill / story consumes existing design-system primitives unchanged / pure backend |
| `react-best-practices` | Complex React feature (data fetching with multiple async dependencies, large list virtualization, hot-path render, image-heavy gallery) / story explicitly fixes a perf bug / new component will live in a hot path (>10 renders/sec or >100ms initial paint) | Trivial UI (label edits, color swaps, copy tweaks) / story already passing perf review with no regression risk / backend-only |
| `composition-patterns` | Story creates OR refactors a component that will be **reused** (going into the design system, used in 2+ places today, or whose API will be consumed by other stories) / story removes boolean-prop proliferation / new context provider | One-off page implementation / story consumes Shadcn primitives unchanged (the primitives already follow these patterns) / backend-only |
| `sharp-edges` | Story introduces a public API surface that downstream code will inherit / new configuration schema consumed by callers / choice of cryptographic library / new external integration whose ergonomics will shape future stories | Internal helper not exposed beyond one package / refactor that doesn't change the API contract / pure UI work / story constrained to existing API patterns |
| `clerk-nextjs-patterns` | **(frontend)** Story modifies `middleware.ts` / adds or changes auth guards on App Router pages or layouts / introduces a Server Action that needs auth / adds an API route under `app/api/**` with auth / caches user-scoped data with `unstable_cache` or `revalidatePath` / changes public-vs-protected route strategy | Pure backend story / story uses existing auth guard patterns unchanged / trivial UI behind already-protected route / no Next.js auth integration touched |
| `clerk-custom-ui` | **(frontend)** Story builds custom sign-in / sign-up / forgot-password pages NOT using Clerk's pre-built components / styles or themes Clerk components (appearance, variables, options) / integrates Clerk visuals with the project's shadcn theme / brands the auth flow with custom palette/typography | Story uses Clerk's pre-built components without customization / no auth UI changes / pure backend / theming already covered by existing `clerk-custom-ui` audit lines in `project-memory.md` |
| `clerk-backend-api` | **(backend)** Story calls Clerk's Backend REST API for user sync, organization provisioning, invitations, metadata operations (public/private/unsafe) / story adds a webhook handler for Clerk events (`user.created`, `organization.updated`, etc.) / story debugs auth issues that require API inspection | Story only uses Clerk client-side (hooks, components in the frontend) / no server-to-Clerk communication / pure UI work / story consumes data already synced by an earlier story |
| `clerk-testing` | **(frontend)** Story adds or modifies E2E tests involving authenticated flows / sets up or changes Playwright auth fixtures (`clerkSetup`, `setupClerkTestingToken`, `storageState`) / introduces a new persona that needs sign-in helpers / migrates between Cypress and Playwright auth setup | Backend-only story / no E2E test component / auth tests already exist for the relevant flow and aren't being modified / story uses existing helper functions (`signInAsUserA`, `restoreUserA`) without touching the fixtures themselves |

**Auth/security-touching stories are the high-value case here**: any story whose ACs mention authentication, authorization, RLS, sessions, JWTs, OAuth, Clerk webhooks, or RBAC should pass through this table and pick up *at least* the relevant Clerk skill — these aren't optional embellishments, they're the difference between a coder who knows the canonical Clerk pattern and one who reinvents it. When in doubt on an auth-adjacent story, ADD the matching Clerk skill rather than SKIP.

Skills you should **never** add to a story's `## Skills` section because they're loaded automatically by their parent agent's frontmatter or by hard-trigger rules in the agent itself:
- `kiat-test-patterns-check` — always loaded by both coders' frontmatter
- `kiat-clerk-auth-review` — hard-triggered by both reviewers when the diff matches Clerk patterns
- `differential-review` — hard-triggered by `kiat-backend-reviewer` Step 3.5 when the diff touches security-critical paths

Listing any of these in `## Skills` is harmless but redundant — and it dilutes the signal that the section is meant to carry (situational, story-scoped skills the coder must actively load).
