...
5. **When and how** `bmad-reconcile` (per story) and `bmad-retrospective`
...
> bmad-reconcile to produce story-NN.reconcile.md.
...
## The `story-NN.reconcile.md` schema (output — written by `bmad-reconcile`)
...
> Generated by `bmad-reconcile` after Team Lead handoff.
...
Append-only by `bmad-reconcile`, force-closed by `bmad-retrospective`,
...
> Append-only queue of L2 proposals from `bmad-reconcile`. Each entry is
> closed either:
...
`Q-NNN` IDs are monotonically allocated — `bmad-reconcile` greps the
...
The protection: at Phase -1 of every new story, the **tech-spec-writer**
scans the queue and asks one question per OPEN L2 entry:
...
| Answer | Action |
|---|---|
| No | Reference the queue entry as context if relevant. Proceed. |
| Yes (overlap) | **AUTO-PROMOTE** the L2 to L3 — change queue status to `PROMOTED`, write an `epic_block` event to `events.jsonl` citing the queue ID, return `SPEC_HANDOFF_FAILED` with reason `"queue_overlap_unresolved"`. Team Lead refuses to launch until the human resolves. |
...
The tech-spec-writer's audit line on every Phase -1:

```
Queue scan: 3 OPEN L2 entries reviewed, 0 overlaps with this story's scope ✓
```

or

```
Queue scan: 3 OPEN L2 entries reviewed, 1 overlap detected (Q-014 affects backend/internal/domain/items) → AUTO-PROMOTED to L3, story handoff failed
```
...
| **`bmad-reconcile`** | Team Lead Phase 5c (after Phase 5b pitfall capture, before Phase 6 rollup) | Story `## Post-Delivery Notes` + queue (read-only context) + `delivery/business/` (read-only context) | `story-NN.reconcile.md` + queue (append L2) + `events.jsonl` (epic_block for L3) + `delivery/business/` or `delivery/specs/` (apply L1) |
| **Tech-spec-writer queue scan** | Phase -1 of every new story | Queue file | nothing if no overlap; queue (status → PROMOTED) + `events.jsonl` (epic_block) on overlap |
| **Team Lead pre-launch L3 check** | Phase 0a / 0b for every new story | `events.jsonl` (search for unresolved `epic_block`) | nothing — refuses to launch on hit |
